...
void writeResult(type_t *u_cur, std::vector<double> &delta, PARAMS params)
{
    ...
    FILE *fp;
    char filename[128];
    sprintf(filename, "output/%s_%dD.bin", iterMth[method], DIM);
    ...
    FILE *fp1;
    char filename1[128];
    sprintf(filename1, "output/%s_%dD_dim_type.txt", iterMth[method], DIM);
    ...
    FILE *fp2;
    char filename2[128];
    sprintf(filename2, "output/%s_%dD_delta.bin", iterMth[method], DIM);
...
void configstcl(INNERS *inners, iterStencil_t *iterstcl, PARAMS *params)
{
    ...
    //                                gaussseidel_1D, gaussseidel_2D, gaussseidel_3D, \
    //                                sor_1D, sor_2D, sor_3D, \
    //                                rbgs_1D, rbgs_2D, rbgs_3D, \
    //                                mgrbgs_1D, mgrbgs_2D, mgrbgs_3D};*/
    switch (DIM) {
        case (1):
            istart = 0;     iend = 1;
            jstart = 0;     jend = 1;
