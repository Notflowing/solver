...
// void smooth(type_t *u, type_t *f, const int level)
// {
//     type_t *u_host = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
//     type_t *f_host = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
//     checkCudaErrors( cudaMemcpy(u_host, u, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
//     checkCudaErrors( cudaMemcpy(f_host, f, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );

//     DIMINFO diminfo = dimsinfo[level];
//     INNERS inners = innersinfo[level];
//     int iter;
//     for (iter = 0; iter < n_cycles; iter++) {
//         MGgaussseidel_2D(u_host, f_host, diminfo, inners);

//         // MGredblackgs_2D(u_host, f_host, diminfo, inners);

//     }

//     checkCudaErrors( cudaMemcpy(u, u_host, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyHostToDevice) );
//     checkCudaErrors( cudaMemcpy(f, f_host, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyHostToDevice) );
...
__global__ void restriction_kernel(type_t * __restrict__ r2, type_t * __restrict__ r, type_t * __restrict__ e2, const int level)
{
    ...
    if (i >= 0 && i < Mdim_coar && j >= 1 && j < Ndim_coar - 1 && k >= 1 && k < Kdim_coar - 1) {
    ...
    // update boundaries
    // Since boundaries are zero, so there is no process!
...
void vcycle(type_t *u, type_t *f, const int level)
{
    ...
    // type_t *u_host = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
    // type_t *f_host = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
    // type_t *r_restest = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
    // checkCudaErrors( cudaMemcpy(u_host, u, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
    // checkCudaErrors( cudaMemcpy(f_host, f, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
    // residual(r_restest, u_host, f_host, level);
    // checkCudaErrors( cudaMemcpy(r, r_restest, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyHostToDevice) );
    // free(u_host);
    // free(f_host);
    // free(r_restest);
    ...
    // type_t *r_hosttest = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
    // type_t *r2_hosttest = (type_t *)malloc(dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t));
    // type_t *e2_host = (type_t *)malloc(dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t));
    // checkCudaErrors( cudaMemcpy(r_hosttest, r, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
    // restriction(r2_hosttest, r_hosttest, e2_host, level + 1);
    // checkCudaErrors( cudaMemcpy(e2, e2_host, dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t), cudaMemcpyHostToDevice) );
    // checkCudaErrors( cudaMemcpy(r2, r2_hosttest, dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t), cudaMemcpyHostToDevice) );
    // free(r_hosttest);
    // free(r2_hosttest);
    // free(e2_host);
    ...
    // type_t *u_hosttest = (type_t *)malloc(dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t));
    // type_t *e2_hosttest = (type_t *)malloc(dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t));
    // checkCudaErrors( cudaMemcpy(u_hosttest, u, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
    // checkCudaErrors( cudaMemcpy(e2_hosttest, e2, dimsinfo[level+1].M*dimsinfo[level+1].N*dimsinfo[level+1].K*sizeof(type_t), cudaMemcpyDeviceToHost) );
    // prolongate(u_hosttest, e2_hosttest, level + 1);
    // checkCudaErrors( cudaMemcpy(u, u_hosttest, dimsinfo[level].M*dimsinfo[level].N*dimsinfo[level].K*sizeof(type_t), cudaMemcpyHostToDevice) );
    // free(u_hosttest);
    // free(e2_hosttest);
